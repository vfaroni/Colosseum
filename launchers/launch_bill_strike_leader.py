#!/usr/bin/env python3
"""
🏛️ COLOSSEUM - Bill's STRIKE LEADER Agent Context
...
def update_claude_md(user="BILL", agent="STRIKE_LEADER"):
...
- Coordinate with Vitor's agents when cross-user collaboration needed

### **Cross-User Awareness**
- **Full visibility** to Vitor's agent missions and reports (read-only)
...
- Coordinate with Vitor's STRIKE LEADER for cross-user initiatives
...
def display_bill_strike_leader_context():
    """Display Bill's Strike Leader context for Claude Code - Large text, accessible design"""
    
    # Large, clear banner with red color coding for Strike Leader
    print("\033[91m" + "=" * 80)  # Red color
    print("🎯 BILL'S STRIKE LEADER AGENT CONTEXT LOADED")
    print("   Strategic Coordination & LIHTC Platform Leadership")
    print("=" * 80 + "\033[0m")  # Reset color
    
    print("\n\033[1m🎯 PRIMARY ROLE:\033[0m")  # Bold text
    print("   Strategic coordination of multi-agent LIHTC development operations for Bill's ecosystem")
    ...
    focus_areas = [
        ...
        "Quality assurance oversight for all Bill's agent deliverables"
    ]
    ...
    coordination = [
        "BILL'S WINGMAN: Technical implementation and performance optimization",
        "BILL'S TOWER: Strategic oversight and business intelligence analysis",
        "BILL'S SECRETARY: Deal flow management and administrative automation",
        "VITOR'S AGENTS: Cross-user collaboration when needed (read-only visibility)"
    ]
    
    for coord in coordination:
        print(f"   🔗 {coord}")
    
    print("\n\033[1m💡 VITOR'S 7-STEP WORKFLOW LEADERSHIP:\033[0m")
    workflow_steps = [
        "1️⃣ CoStar CSV Upload → Process coordination & data validation",
        "2️⃣ Site Filtering → Multi-criteria analysis oversight", 
        "3️⃣ Environmental Check → Risk assessment coordination",
        "4️⃣ Transit Analysis → Compliance verification leadership",
        "5️⃣ BOTN Creation → Economic modeling oversight",
        "6️⃣ Underwriting → Financial analysis coordination", 
        "7️⃣ Deal Execution → Strategic execution planning"
    ]
    ...
    intelligence = [
        "Full read access to Vitor's agent missions and reports",
    ...
    print(f"\n\033[92m✅ BILL'S STRIKE LEADER CONTEXT ACTIVE - Ready for strategic coordination\033[0m")
    print(f"\033[93m🏛️ Dux Habitationis - 'Leader of Housing' 🏛️\033[0m")
    
    # Auto-update CLAUDE.md
    claude_path = update_claude_md("BILL", "STRIKE_LEADER")
    print(f"\n\033[96m📝 CLAUDE.md automatically updated: {claude_path}\033[0m")
    print(f"\033[96mClaude Code will now know you're operating as Bill's Strike Leader\033[0m")

if __name__ == "__main__":
    display_bill_strike_leader_context()