...
def is_yellow_cell(cell):
    ...
    This function handles different Excel formats safely.
    """
    try:
        # Common representations of yellow in Excel
        yellow_indices = ['FFFF00', 'FFFF00FF', 'FFFFFF00', 6]
        yellow_strings = ['ffff00', 'ffff00ff', 'ffffff00']
        
        # Check fill_type first (if it exists)
        if hasattr(cell.fill, 'fill_type') and cell.fill.fill_type != 'none':
            # Various ways Excel might represent the color
            
            # Method 1: Check start_color.index
            if hasattr(cell.fill, 'start_color') and hasattr(cell.fill.start_color, 'index'):
                fill_color = cell.fill.start_color.index
                if fill_color in yellow_indices or str(fill_color).lower() in yellow_strings:
                    return True
            
            # Method 2: Check start_color.rgb (as string)
            if hasattr(cell.fill, 'start_color') and hasattr(cell.fill.start_color, 'rgb'):
                rgb = cell.fill.start_color.rgb
                if rgb and (rgb in yellow_indices or str(rgb).lower() in yellow_strings):
                    return True
                # Some RGB objects have a value attribute
                if hasattr(rgb, 'value'):
                    rgb_value = rgb.value
                    if rgb_value and (rgb_value in yellow_indices or str(rgb_value).lower() in yellow_strings):
                        return True
            
            # Method 3: Check fgColor attribute
            if hasattr(cell.fill, 'fgColor'):
                # Check fgColor.rgb
                if hasattr(cell.fill.fgColor, 'rgb'):
                    fg_rgb = cell.fill.fgColor.rgb
                    if fg_rgb:
                        # Handle different RGB representations
                        if isinstance(fg_rgb, str) and (fg_rgb.lower().startswith('ffff') or fg_rgb in yellow_indices):
                            return True
                        # Some RGB objects have a value attribute
                        if hasattr(fg_rgb, 'value'):
                            fg_rgb_value = fg_rgb.value
                            if fg_rgb_value and (fg_rgb_value in yellow_indices or str(fg_rgb_value).lower() in yellow_strings):
                                return True
                
                # Check fgColor.index
                if hasattr(cell.fill.fgColor, 'index'):
                    fg_index = cell.fill.fgColor.index
                    if fg_index in yellow_indices or str(fg_index).lower() in yellow_strings:
                        return True
            
            # Method 4: Check for common patterns in Excel's color coding
            if cell.fill.patternType == 'solid':
                # Some versions use different color representations
                for color_attr in ['bgColor', 'fgColor']:
                    if hasattr(cell.fill, color_attr):
                        color_obj = getattr(cell.fill, color_attr)
                        # Try different color value attributes
                        for value_attr in ['rgb', 'theme', 'indexed', 'auto', 'value']:
                            if hasattr(color_obj, value_attr):
                                color_value = getattr(color_obj, value_attr)
                                if color_value and (color_value in yellow_indices or 
                                                  str(color_value).lower() in yellow_strings):
                                    return True
        
        return False
    except Exception as e:
        # If any error occurs in color checking, log it but don't fail
        logger.debug(f"Error checking cell color: {str(e)}")
        return False
