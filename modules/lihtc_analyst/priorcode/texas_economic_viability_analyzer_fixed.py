#!/usr/bin/env python3
"""
Texas LIHTC Economic Viability Analyzer - FIXED VERSION
Uses HUD AMI GeoPackage for accurate county data lookups
...
class TexasEconomicViabilityAnalyzer:
    ...
    def load_data(self):
        ...
        # Create county lookup dictionary for fast access
        self.county_lookup = {}
        for idx, row in self.texas_hud_gdf.iterrows():
            # Multiple lookup keys for flexibility
            base_name = row['county_name_census']  # e.g., "Harris"
            ...
            # Store by various formats for flexible lookup
            for key in [base_name.upper(), full_name.upper(), 
                       base_name.lower(), full_name.lower(),
                       base_name, full_name]:
                self.county_lookup[key] = row_dict
    ...
    def calculate_economic_metrics(self, row):
        ...
        try:
            ...
            # Look up county in our HUD data using flexible matching
            county_data = None
            for lookup_key in [county.upper(), f"{county} County".upper(), county]:
                if lookup_key in self.county_lookup:
                    county_data = self.county_lookup[lookup_key]
                    break
            
            if not county_data:
                print(f"Warning: No HUD data found for county: {county}")
                return self._empty_metrics()
            
            # Get 2BR 60% AMI rent
            rent_2br_60 = county_data.get('rent_2br_60pct', 0)
            if pd.isna(rent_2br_60) or rent_2br_60 == 0:
                print(f"Warning: No 2BR 60% rent data for {county}")
                return self._empty_metrics()
            ...
            return {
                ...
                'HUD_Area_Name': county_data.get('hud_area_name', 'Unknown'),
                'Metro_Status': county_data.get('metro_status', 'Unknown')
            }
    ...
    def _empty_metrics(self):
        ...
        return {col: 0 for col in [
            ...
            'Simple_ROI_Percent', 'Economic_Score', 'HUD_Area_Name', 'Metro_Status'
        ]}
    ...
    def _print_key_findings(self):
        ...
        for idx, row in top_sites.iterrows():
            ...
            print(f"  HUD Area: {row['HUD_Area_Name']}")
    ...
    def run(self):
        ...
        print("Starting Texas Economic Viability Analysis (FIXED VERSION)...")
        print("="*50)
