            ...
            self.logger.info("   🧮 Extracting comprehensive basis and credits (merged cell aware)...")
            
            # Enhanced search terms for basis and credits
            basis_terms = {
                'eligible_basis': [
                    'total eligible basis', 'eligible basis', 'aggregate eligible basis',
                    'total development cost eligible basis', 'eligible development cost'
                ],
                'qualified_basis': [
                    'qualified basis', 'total qualified basis', 'aggregate qualified basis',
                    'applicable fraction qualified basis', 'low income qualified basis'
                ],
                'federal_credits_annual': [
                    'annual federal credit', 'federal tax credit annual', 'annual federal',
                    'federal credit amount', 'ltc federal annual'
                ],
                'state_credits_annual': [
                    'annual state credit', 'state tax credit annual', 'annual state',
                    'state credit amount', 'ltc state annual'
                ],
                'federal_credits_total': [
                    'total federal credit', 'federal credit total', 'total federal tax credit',
                    'federal ltc total', '10 year federal credit'
                ],
                'state_credits_total': [
                    'total state credit', 'state credit total', 'total state tax credit',
                    'state ltc total', '4 year state credit'
                ],
                'gross_rent_potential': [
                    'gross rent potential', 'potential gross rent', 'gross rental income',
                    'total gross rent', 'gross potential income'
                ],
                'effective_gross_income': [
                    'effective gross income', 'effective gross', 'adjusted gross income',
                    'total effective gross income'
                ],
                'operating_expenses': [
                    'operating expenses', 'total operating expense', 'operating costs',
                    'annual operating expenses'
                ],
                'net_operating_income': [
                    'net operating income', 'noi', 'net income', 'annual noi'
                ]
            }
            
            # Scan the entire sheet with merged cell awareness
            for row in range(1, 200):
                # Get all values in this row, handling merged cells
                row_values = self.scan_merged_cell_area(sheet, row, row, 0, 15)  # Columns A-P
                
                # Look for basis/credit terms in any cell in this row
                for cell_ref, cell_value in row_values.items():
                    if isinstance(cell_value, str):
                        ...
                        for key, terms in basis_terms.items():
                            if any(term in cell_lower for term in terms):
                                # Found a match! Now look for numeric values in this row
                                numeric_values = []
                                for val_ref, val in row_values.items():
                                    if isinstance(val, (int, float)) and val > 0:
                                        # Filter out obviously wrong values
                                        if val > 1000:  # Basis/credits should be substantial amounts
                                            numeric_values.append({
                                                'cell': val_ref,
                                                'value': val
                                            })
                                
                                # Take the largest numeric value as most likely correct
                                if numeric_values:
                                    best_value = max(numeric_values, key=lambda x: x['value'])
                                    basis_credits[key] = float(best_value['value'])
                                    
                                    # Store detailed information
                                    basis_credits['credit_calculations'][f'{key}_details'] = {
                                        'source_cell': cell_ref,
                                        'value_cell': best_value['cell'],
                                        'description': cell_value[:100],
                                        'amount': float(best_value['value']),
                                        'all_values_found': numeric_values
                                    }
                                    
                                    self.logger.info(f"     Found {key}: ${best_value['value']:,.0f} at {best_value['cell']}")
                                    break
                        
                        # Also look for percentage values (applicable fraction)
                        if 'applicable fraction' in cell_lower or 'low income percentage' in cell_lower:
                            for val_ref, val in row_values.items():
                                if isinstance(val, (int, float)) and 0 < val <= 1:
                                    basis_credits['applicable_fraction'] = float(val)
                                    break
                                elif isinstance(val, (int, float)) and 1 < val <= 100:
                                    basis_credits['applicable_fraction'] = float(val) / 100
                                    break
            
            # Additional search for credit percentages
            for row in range(1, 200):
                ...
                for cell_ref, cell_value in row_values.items():
                    if isinstance(cell_value, str):
                        ...
                        # Look for credit percentages
                        if any(term in cell_lower for term in ['9%', '4%', 'credit percentage', 'credit rate']):
                            for val_ref, val in row_values.items():
                                if isinstance(val, (int, float)):
                                    if 0.03 <= val <= 0.10:  # 3% to 10% as decimal
                                        basis_credits['credit_percentage'] = float(val)
                                    elif 3 <= val <= 10:  # 3% to 10% as whole number
                                        basis_credits['credit_percentage'] = float(val) / 100
                                    break
    ...
    def process_single_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        """Process a single Excel file with comprehensive extraction - enhanced for merged cells"""
        try:
            self.logger.info(f"📂 Processing file: {file_path.name}")
            
            parsed_info = self.parse_filename(file_path.name)
            
            # Load workbook with merged cell detection
            workbook = openpyxl.load_workbook(file_path, data_only=True)
            
            self.logger.info(f"   📋 Available sheets: {', '.join(workbook.sheetnames)}")
            self.logger.info("   🔧 Enhanced extraction with merged cell handling enabled")
            ...
            result = {
                'metadata': {
                    ...
                    'extraction_method': 'comprehensive_enhanced_merged_cells',
            ...
            # Execute comprehensive extractions with enhanced merged cell handling
            extraction_results = {}
            ...
            # 3. Enhanced Scoring Breakdown (merged cell aware)
            self.logger.info("   📊 Extracting enhanced scoring breakdown...")
            ...
            # 4. Enhanced Basis and Credits (merged cell aware)
            self.logger.info("   🧮 Extracting enhanced basis and credits...")
            ...
            # Enhanced logging for merged cell improvements
            scoring_points = result['scoring_breakdown'].get('total_points_claimed', 0)
            basis_found = sum(1 for key in ['eligible_basis', 'qualified_basis', 'federal_credits_annual'] 
                            if result['basis_and_credits'].get(key) is not None)
            ...
            self.logger.info(f"   🎯 Scoring points found: {scoring_points}")
            self.logger.info(f"   🧮 Basis/credit values found: {basis_found}/3")
    ...
    def process_files(self, limit: int = None, test_mode: bool = True):
        ...
        print("🎯 CTCAC COMPREHENSIVE EXTRACTOR - ENHANCED VERSION 2.1")
        print("=" * 70)
        ...
        print("   • ENHANCED: Merged cell handling for accurate extraction")
        print("=" * 70)
        ...
        for i, file_path in enumerate(excel_files, 1):
            try:
                ...
                if result:
                    ...
                    scoring_points = result['scoring_breakdown'].get('total_points_claimed', 0)
                    total_scoring_points += scoring_points
                    
                    basis_found = sum(1 for key in ['eligible_basis', 'qualified_basis', 'federal_credits_annual'] 
                                    if result['basis_and_credits'].get(key) is not None)
                    total_basis_found += basis_found
                    
                    print(f"   ✅ SUCCESS - Data richness: {data_richness}/100, Points: {scoring_points}, Basis: {basis_found}/3")
                else:
                    failed_count += 1
                    print(f"   ❌ FAILED")
        ...
        print("\n" + "=" * 70)
        print("📊 COMPREHENSIVE PROCESSING SUMMARY - ENHANCED VERSION 2.1")
        print("=" * 70)
        ...
        if processed_count > 0:
            ...
            avg_scoring = total_scoring_points / processed_count
            avg_basis = total_basis_found / processed_count
            print(f"📊 Average data richness: {avg_richness:.1f}/100")
            print(f"🎯 Average scoring points found: {avg_scoring:.1f}")
            print(f"🧮 Average basis/credit values found: {avg_basis:.1f}/3")
        ...
        print(f"\n🎯 ENHANCED DATA EXTRACTED FOR RAG SYSTEM:")
        ...
        print("  ✅ Sources and basis breakdown details")
        print("  ✅ SCE/FCE calculations (9% applications)")
        print("  🆕 MERGED CELL HANDLING for accurate extraction")
        
        print(f"\n🚀 ENHANCEMENT IMPROVEMENTS:")
        print("  • Better scoring point detection")
        print("  • Enhanced basis & credit extraction")
        print("  • Improved unit mix identification")
        print("  • Merged cell awareness throughout")
        
        self.logger.info(f"Enhanced comprehensive extraction complete. Success: {processed_count}, Failed: {failed_count}")
...
def main():
    ...
    print("🎯 CTCAC COMPREHENSIVE DATA EXTRACTOR - ENHANCED v2.1")
    print("=" * 60)
    print("Enhanced version with MERGED CELL HANDLING that extracts:")
    ...
    print("• MERGED CELL awareness for CTCAC applications")
    print("=" * 60)
    ...
    print("\n🎉 Enhanced comprehensive extraction complete!")
    print("💡 Your JSON files now contain significantly more LIHTC data!")
    ...
    print("   - Each JSON now contains comprehensive LIHTC data with merged cell handling!")
...
class CTCACComprehensiveExtractor:
    ...
    def extract_comprehensive_basis_credits(self, workbook) -> Dict[str, Any]:
        """Extract comprehensive basis and credit calculations - enhanced for merged cells"""
        try:
