...
class EnhancedTexasAnalyzer:
            ...
            if len(data) > 1:
                ...
                def safe_int(value, default=0):
                    try:
                        ...
                        # FIXED: Catch all variations of the -666 error pattern
                        if str_val.startswith('-666') or '666666666' in str_val:
                            print(f"    ⚠️ Census API -666 error pattern detected: {str_val} - treating as unavailable")
                            return default
                        ...
                        # ENHANCED: Much stricter bounds checking
                        if int_val < -500 or int_val > 2000000:  # Very reasonable bounds
                            print(f"    ⚠️ Census API unreasonable value: {int_val:,} - treating as unavailable")
                            return default
                        
                        # Additional check for negative values where they shouldn't be
                        if int_val < 0:
                            print(f"    ⚠️ Census API negative value: {int_val:,} - treating as unavailable")
                            return default
                ...
                # ENHANCED: Better validation for median income
                if median_income <= 0 or median_income > 500000:  # Very reasonable income bounds
    ...
    def get_nearby_lihtc_projects(self, target_point: Point, radius_miles: float) -> List[Dict]:
        """
        ENHANCED: Find LIHTC projects within specified radius with better project details
        """
                ...
                if project_coords:
                    ...
                    try:
                        ...
                        if distance_miles <= radius_miles:
                            # ENHANCED: Better project name handling
                            project_name = str(project.get('PROJECT_NAME', 'Unknown')).strip()
                            if project_name in ['Unknown', 'nan', 'None', '']:
                                project_name = str(project.get('PROJECT_NAM', 'Unknown')).strip()  # Try alternate column
                            
                            project_info = {
                                'distance': round(distance_miles, 2),
                                'project_name': project_name,
                                'total_units': int(project.get('TOTAL_UNITS', 0)) if pd.notna(project.get('TOTAL_UNITS', 0)) else 0,
                                'year_placed': int(project.get('YEAR_PLACED_IN_SERVICE', 0)) if pd.notna(project.get('YEAR_PLACED_IN_SERVICE', 0)) else 0,
...
"""
                    for project in display_projects:
                        ...
                        year_placed = project.get('year_placed', 0)
                        
                        html_content += f"""
                                <div class="project-item">
                                    <span class="project-name" title="{project.get('project_name', 'Unknown')}">{project_name}</span>
                                    <span class="project-details">{total_units} units, {distance} mi</span>
                                </div>
"""
