#!/usr/bin/env python3
"""
TDHCA ACCURATE Texas LIHTC Analyzer
Implements correct TDHCA distance requirements from 2025 QAP
"""
...
class TDHCAAccurateAnalyzer:
    def __init__(self, 
                 ...
                 work_dir: str = "./tdhca_accurate_analysis"):
        """
        ...
        try:
            ...
            for _, row in texas_df.iterrows():
                ...
                self.ami_data[fips] = {
                    ...
                    'rent_limits': {
                        '60_pct': {
                            '1br': row['1BR 60%'], '2br': row['2BR 60%'], '3br': row['3BR 60%']
                        }
                    }
                }
    ...
    def check_one_mile_three_year_rule(self, point: Point, target_population: str, current_year: int = 2026) -> Dict:
        ...
        Returns competing projects within 1 mile from prior 3 years
        """
    ...
    def analyze_site(self, address: str, lat: float = None, lon: float = None) -> Dict:
        """Comprehensive site analysis for both 4% and 9% deals"""
        ...
        print("   DEBUG: About to check QCT/DDA...")
        print(f"   DEBUG: QCT data type: {type(self.qct_data)}")
        print(f"   DEBUG: DDA data type: {type(self.dda_data)}")
       ...
       # Check QCT/DDA status
        ...
        try:
            if self.qct_data is not None:
                print(f"      DEBUG: QCT data has {len(self.qct_data)} features")
                qct_intersects = self.qct_data[self.qct_data.contains(point)]
                qct_status = len(qct_intersects) > 0
                print(f"      DEBUG: QCT check completed - status: {qct_status}")
        except Exception as e:
            print(f"      ERROR in QCT check: {e}")
            qct_status = False

        # Check DDA
        try:
            print("      DEBUG: Starting DDA check...")
            if self.dda_data is not None:
                print(f"      DEBUG: DDA is not None, checking length...")
                dda_length = len(self.dda_data)
                print(f"      DEBUG: DDA data has {dda_length} features")
                if dda_length > 0:
                    dda_intersects = self.dda_data[self.dda_data.contains(point)]
                    dda_status = len(dda_intersects) > 0
                    print(f"      DEBUG: DDA check completed - status: {dda_status}")
                else:
                    print(f"      DEBUG: DDA data is empty")
                    dda_status = False
            else:
                print(f"      DEBUG: DDA data is None")
                dda_status = False
        except Exception as e:
            print(f"      ERROR in DDA check: {e}")
            import traceback
            traceback.print_exc()
            dda_status = False

        print(f"   DEBUG: QCT/DDA checks complete - QCT: {qct_status}, DDA: {dda_status}")
        
        # Determine county
        print("   DEBUG: Determining county...")
        county_name = "Unknown"
        ...
        print(f"   DEBUG: County determined: {county_name}")

        # Run TDHCA compliance checks
        target_population = 'General'
        print("   DEBUG: Starting TDHCA compliance checks...")

        try:
            print("   DEBUG: About to check one_mile_three_year_rule...")
            one_mile_check = self.check_one_mile_three_year_rule(point, target_population)
            print("   DEBUG: one_mile_check completed")
        except Exception as e:
            print(f"   ERROR in one_mile_check: {e}")
            import traceback
            traceback.print_exc()
            raise

        try:
            print("   DEBUG: About to check two_mile_same_year_rule...")
            two_mile_check = self.check_two_mile_same_year_rule(point, county_name)
            print("   DEBUG: two_mile_check completed")
        except Exception as e:
            print(f"   ERROR in two_mile_check: {e}")
            import traceback
            traceback.print_exc()
            raise
        ...
        try:
            print("   DEBUG: About to calculate TDHCA same tract score...")
            tdhca_score = self.calculate_tdhca_same_tract_score(census_tract, target_population)
            print(f"   DEBUG: TDHCA score calculated: {tdhca_score}")
        except Exception as e:
            print(f"   ERROR in TDHCA score calculation: {e}")
            import traceback
            traceback.print_exc()
            raise
        if census_tract and len(census_tract) >= 5:
            county_fips = census_tract[2:5]
            # Map to county name (would need full mapping)
            county_map = {
                '201': 'Harris',
                '113': 'Dallas',
                '439': 'Tarrant',
        ...
        # Run TDHCA compliance checks
        target_population = 'General'  # Could be enhanced to detect from description
        ...
        analysis = {
            ...
            'one_mile_three_year': one_mile_check,
    ...
    def process_costar_data(self, costar_file: str, address_column: str = 'Address') -> pd.DataFrame:
        """Process CoStar data with TDHCA analysis"""
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                if 'error' not in analysis:
                    ...
                    # Competition data
                    one_mile = analysis.get('one_mile_three_year', {})
                    df.at[idx, 'One_Mile_Competitors'] = len(one_mile.get('competing_projects', []))
    ...
    def save_results(self, df: pd.DataFrame, base_filename: str):
        """Save results to Excel with separate sheets for 4% and 9% rankings"""
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            summary_data = {
                'Metric': [
                    ...
                    'Properties in QCT', 'Properties in DDA',
...
def main():
    ...
    POSITIONSTACK_API_KEY = "41b80ed51d92978904592126d2bb8f7e"  # From your other script
    ...
    COSTAR_FILE = "CS_Land_TX110ac_05312025_RANKED_4p_9p.xlsx"  # The file you want to analyze
    ...
    # Process CoStar data
    results_df = analyzer.process_costar_data(COSTAR_FILE, address_column='Address')
    
    # Save results
    output_file = analyzer.save_results(results_df, 'CoStar_TX_Land')
    ...
    # Show top properties for each category
    print("\n🏆 TOP 4% BOND PROPERTIES:")
    top_4p = results_df[results_df['4pct_Category'] == 'VIABLE'].head(5)
    for idx, row in top_4p.iterrows():
        print(f"   • {row['Address']} - QCT/DDA: {row['QCT_Status'] or row['DDA_Status']}")
    
    print("\n🏆 TOP 9% LIHTC PROPERTIES:")
    top_9p = results_df[results_df['9pct_Category'] == 'COMPETITIVE'].head(5)
    for idx, row in top_9p.iterrows():
        print(f"   • {row['Address']} - Points: {row['9pct_Points_Est']}")
