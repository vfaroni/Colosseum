...
class TexasHUDAnalyzer:
                    ...
                    if poverty_rate is not None:
                        try:
                            ...
                            if result["low_poverty_tract"]:
                                ...
                                print(f"  ❌ Low Poverty Status: NOT QUALIFIED (>20%)")
    ...
    def suggest_address_fixes(self, failed_addresses: List[str]) -> Dict[str, List[str]]:
        ...
        for address in failed_addresses:
            ...
            # Try with ZIP code patterns
            if not re.search(r'\d{5}', address):
                address_suggestions.append(f"{address} 75201")  # Dallas ZIP as example
    ...
    def batch_analyze_with_retry(self, addresses: List[str]) -> pd.DataFrame:
        ...
        # Retry failed addresses with suggestions
        if failed_addresses:
            print(f"\n🔄 Retrying {len(failed_addresses)} failed addresses with suggested fixes...")
            suggestions = self.suggest_address_fixes(failed_addresses)
            
            for orig_address in failed_addresses:
                print(f"\n--- Retrying: {orig_address} ---")
                suggested_addresses = suggestions.get(orig_address, [])
                
                success = False
                for suggested in suggested_addresses:
                    print(f"  Trying suggestion: {suggested}")
                    ...
                    if 'error' not in retry_result:
                        ...
                        for result in results:
                            if result.get('address') == orig_address and 'error' in result:
                                ...
                                result['address'] = orig_address  # Keep original address
                ...
                if not success:
                    print(f"  ❌ All retry attempts failed for: {orig_address}")
...
"""
        ...
        for _, row in results_df.iterrows():
            if pd.isna(row.get('longitude')):
                ...
                text_report += f"\n   Status: {row['analysis_summary']}"
                if not pd.isna(row.get('poverty_rate')):
                    text_report += f"\n   Poverty Rate: {row['poverty_rate']:.2f}%"
...
"""
        ...
        for _, row in results_df.iterrows():
            if pd.isna(row.get('longitude')):
                ...
                rtf_content += f"\\line    Status: \\cf4\\b {row['analysis_summary']}\\b0\\cf1"
                if not pd.isna(row.get('poverty_rate')):
                    rtf_content += f"\\line    Poverty Rate: {row['poverty_rate']:.2f}%"
                if row.get('matched_address') and row['matched_address'] != row['address']:
...
if __name__ == "__main__":
    ...
    if analyzer.load_hud_data():
        ...
        # For batch processing with enhanced retry:
        # addresses = ["Your first address", "Your second address", "Your third address"]
        # results_df = analyzer.batch_analyze_with_retry(addresses)
        # 
        # Generate both TXT and RTF reports
        # report = analyzer.create_summary_report(results_df, format_type='both')
        # 
        # Or just RTF:
        # report = analyzer.create_summary_report(results_df, format_type='rtf')
        # print(report)
    else:
