#!/usr/bin/env python3
"""
ENHANCED CTCAC QAP OCR Extractor with RAG Optimization and 9%/4% Program Tagging
FIXES: Sentence continuity across page breaks, complete section validation
...
class EnhancedCTCACQAPExtractor:
    """Extract and format CTCAC QAP with RAG optimization and improved sentence continuity"""
    ...
    def clean_text_for_continuity(self, text: str) -> str:
        """Clean text and prepare for sentence continuity analysis"""
        # Remove page numbers and footers that interfere with sentence detection
        text = re.sub(r'\n\s*\d+\s*\n', '\n', text)
        text = re.sub(r'\n\s*Regulations\s*\n', '\n', text)
        text = re.sub(r'\n\s*Section \d+.*?\n', '\n', text)
        return text.strip()
    ...
    def is_sentence_complete(self, text: str) -> bool:
        ...
        # Remove trailing whitespace and check last characters
        text = text.strip()
        ...
        # Check for list items or numbered items that might be complete
        if re.search(r'[a-z]\)\s*$', text) or re.search(r'\d+\)\s*$', text):
            return True
    ...
    def merge_incomplete_sentences(self, pages: List[Tuple[int, str]]) -> List[Tuple[int, str]]:
        """Merge text across pages to preserve sentence continuity but preserve section boundaries"""
        if not pages:
            return pages
        ...
        for i, (page_num, text) in enumerate(pages):
            clean_text = self.clean_text_for_continuity(text)
            
            # Check if this page starts a new section (don't merge across sections)
            has_section_header = bool(re.search(r'Section\s+(\d+)\.\s+([^\n]+)', clean_text))
            
            # If this is not the first page, previous page had incomplete sentence,
            # and this page doesn't start a new section
            if (i > 0 and merged_pages and 
                not has_section_header and 
                not self.is_sentence_complete(merged_pages[-1][1])):
                
                prev_page_num, prev_text = merged_pages[-1]
                
                # Only merge if the incomplete sentence seems to continue
                # (avoid merging completely unrelated content)
                prev_words = prev_text.strip().split()[-3:]  # Last few words
                next_words = clean_text.strip().split()[:3]   # First few words
                
                # Simple heuristic: merge if last word doesn't end with punctuation
                # and next text doesn't start with capital letter (indicating new sentence)
                should_merge = (prev_words and 
                              not prev_words[-1].endswith(('.', '!', '?', ':')) and
                              next_words and 
                              not next_words[0][0].isupper())
                
                if should_merge:
                    # Merge with previous page
                    merged_text = prev_text.rstrip() + " " + clean_text.lstrip()
                    merged_pages[-1] = (prev_page_num, merged_text)
                    
                    # Log the merge for validation
                    self.validation_issues.append({
                        "type": "sentence_merge",
                        "pages": f"{prev_page_num}-{page_num}",
                        "preview": prev_text[-30:] + " [MERGED] " + clean_text[:30]
                    })
                else:
                    merged_pages.append((page_num, clean_text))
            else:
                merged_pages.append((page_num, clean_text))
        
        return merged_pages
    
    def validate_section_completeness(self, section_num: str, content: str) -> bool:
        """Validate that a section appears complete"""
        issues = []
        
        # Check for common incompleteness indicators
        if content.strip().endswith(('94', '95', '96', '97', '98', '99')):
            issues.append("Ends with page number - likely incomplete")
            
        # Check for incomplete sentences
        if not self.is_sentence_complete(content):
            issues.append("Ends with incomplete sentence")
            
        # Check for minimum content length (very short sections might be incomplete)
        if len(content.strip()) < 100:
            issues.append("Section content very short - possibly incomplete")
            
        # Look for mid-word breaks
        if re.search(r'\w-\s*$', content.strip()):
            issues.append("Ends with hyphenated word break")
...
#
# ENHANCED: Fixed sentence continuity across page breaks and section validation
################################################################################
...
"""
    ...
    def create_chunk(self, text: str, section: str, programs: List[str], 
        ...
        topic_keywords = {
            ...
            "debt-service": ["debt service", "DCR", "coverage ratio"]
        }
        ...
        chunk = {
            ...
            "metadata": {
                ...
                "sentence_complete": self.is_sentence_complete(text)
            }
        }
    ...
    def process_section(self, section_num: str, title: str, content: str, 
        ...
        is_complete = self.validate_section_completeness(section_num, content)
        ...
        # Add validation warning if incomplete
        if not is_complete:
            output.append("[WARNING: SECTION MAY BE INCOMPLETE - SEE VALIDATION LOG]")
        ...
        output.append(f"- Validation Status: {'COMPLETE' if is_complete else 'INCOMPLETE'}")
    ...
    def save_metadata(self):
        ...
        with open(self.metadata_dir / "validation_log.json", 'w') as f:
    ...
    def run(self):
        ...
        print(f"Starting Enhanced CTCAC QAP extraction from: {self.pdf_path}")
        ...
        # ENHANCEMENT: Merge incomplete sentences across pages
        print("Enhancing sentence continuity across page breaks...")
        merged_pages = self.merge_incomplete_sentences(text_pages)
        
        total_pages = len(merged_pages)
        
        # Create main output file
        output_file = self.output_dir / "CTCAC_QAP_2025_OCR_RAG_ENHANCED.txt"
            ...
            for page_num, text in merged_pages:
                ...
                if section_match:
                    ...
                    if current_section:
                        ...
                        section_file = self.sections_dir / f"{section_num}_{safe_title}.txt"
        ...
        print(f"Total pages processed: {len(text_pages)}")
        print(f"Pages after merging: {total_pages}")
        ...
        summary = {
            ...
            "total_pages_original": len(text_pages),
            ...
            "enhancements": [
                "Sentence continuity across page breaks",
        ...
        with open(self.output_dir / "extraction_summary_enhanced.json", 'w') as f:
            json.dump(summary, f, indent=2)
        
        print(f"\nFor RAG usage, see extraction_summary_enhanced.json")
        
        if self.validation_issues:
            print(f"\nValidation issues found - see metadata/validation_log.json")
            print("Key issues:")
            for issue in self.validation_issues[:3]:  # Show first 3 issues
                print(f"  - {issue['type']}: {issue.get('section', issue.get('pages', 'N/A'))}")
...
def main():
    ...
    output_dir = "/Users/williamrice/HERR Dropbox/Bill Rice/Structured Consultants/AI Projects/CTCAC_RAG/processed_data/CTCAC_QAP_2025_RAG_ENHANCED"
    
    extractor = EnhancedCTCACQAPExtractor(pdf_path, output_dir)
    extractor.run()
