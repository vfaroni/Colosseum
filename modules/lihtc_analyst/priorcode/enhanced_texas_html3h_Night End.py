#!/usr/bin/env python3
"""
Enhanced Texas LIHTC Analyzer - FINAL FIXED VERSION
Fixes: Census API errors, scoring logic, poverty indicators, nearby project listings
"""
...
class EnhancedTexasAnalyzer:
    ...
    def load_hud_designation_data(self):
        """Load existing QCT/DDA data and new comprehensive LIHTC dataset"""
        try:
    ...
    def print_analysis_summary(self, result: Dict):
        """FIXED: Print formatted analysis summary with correct income display"""
        ...
        if result.get("competition_analysis"):
            ...
            if comp['nearest_project']:
                ...
                # FIXED: Show actual project details if available
...
# FIXED: Update HTML report generation to show correct project details and income 
    ...
    def calculate_opportunity_index_score(self, demographics: Dict, ami_data: Dict) -> Dict:
        """FIXED: Calculate Texas opportunity index score with proper low poverty recognition"""
    ...
    def determine_market_radius(self, point: Point, demographics: Dict = None) -> float:
        ...
        Returns radius in miles based on:
        - Dense Urban (Houston, Dallas, Austin): 2 miles
        - Other Metro Areas: 3 miles  
        - Suburban: 4 miles
        - Rural: 8 miles
        """
    ...
    def get_nearby_lihtc_projects(self, target_point: Point, radius_miles: float) -> List[Dict]:
        """
        FIXED: Find LIHTC projects with correct column names and year formatting
        """
    ...
    def calculate_market_saturation_score(self, nearby_projects: List[Dict], demographics: Dict) -> Dict:
        ...
        Formula: (Total LIHTC Units in Area) / (Income-Qualified Households) * 100
        
        Saturation Levels:
        - LOW: < 15 units per 100 qualified households (3 points)
        - MEDIUM: 15-30 units per 100 qualified households (2 points)  
        - HIGH: 30-50 units per 100 qualified households (1 point)
        - OVERSATURATED: > 50 units per 100 qualified households (0 points)
        """
        
        if not nearby_projects:
            return {
                'saturation_level': 'NONE',
                'saturation_score': 3,  # ✅ BEST score for no competition!
    ...
    def analyze_lihtc_competition(self, point: Point, demographics: Dict) -> Dict:
        """
        Comprehensive LIHTC competition analysis for TDHCA scoring
        """
    ...
    def calculate_enhanced_texas_lihtc_score(self, analysis_result: Dict) -> Dict:
        """
        FIXED: Enhanced Texas LIHTC scoring with proper category logic
        """
        ...
        # FIXED: Enhanced Category Logic with proper differentiation
        ...
        # FIXED: Much better category assignment logic
        if has_federal_boost and opp_points >= 2 and comp_score >= 3:
            scoring["category"] = "BEST"  # Federal + excellent opportunity + excellent market
        elif has_federal_boost and opp_points >= 2:
            scoring["category"] = "BEST"  # Federal + excellent opportunity (low poverty + good income)
        elif has_federal_boost and total_points >= 4:
            scoring["category"] = "GOOD"   # Federal benefits + good total score
        elif has_federal_boost:
            scoring["category"] = "GOOD"   # Federal benefits make it worthwhile
        elif opp_points >= 2 and comp_score >= 2:
            scoring["category"] = "GOOD"   # Excellent characteristics without federal benefits
        elif total_points >= 3:
            scoring["category"] = "MAYBE"  # Some positive factors
        elif comp_score == 0:  # Oversaturated market
            scoring["category"] = "FIRM NO"  # Avoid oversaturated markets
        else:
            scoring["category"] = "FIRM NO"  # No significant benefits
        
        # Add reasoning for category assignment
        if scoring["category"] == "BEST":
            scoring["scoring_details"].append("Category: BEST - Excellent opportunity characteristics + federal benefits")
            ...
            scoring["scoring_details"].append("Category: MAYBE - Mixed factors, proceed with caution")
        else:
            scoring["scoring_details"].append("Category: FIRM NO - Insufficient LIHTC advantages or market issues")
    ...
    def print_analysis_summary(self, result: Dict):
        """Print formatted analysis summary with poverty indicators"""
        print(f"  📍 Location: {result['coordinates']['latitude']:.6f}, {result['coordinates']['longitude']:.6f}")
        ...
        if result.get("demographics"):
            ...
            print(f"  📊 Demographics: Poverty {poverty_rate}% {poverty_indicator}, Income ${demo.get('median_household_income', 0):,} {quality_emoji}")
...
"""
    ...
    def create_html_report(self, results: List[Dict]) -> str:
        """ENHANCED: Create professional HTML report with nearby project listings"""
        ...
        # Create HTML content with all fixes
...
"""
        ...
        # Add detailed results with all enhancements
        for i, result in enumerate(results, 1):
            if "error" in result:
                html_content += f"""
            <div class="property">
...
"""
            ...
            # ENHANCED: Market Analysis Section with Project Listings
            if comp_analysis:
                html_content += f"""
                        <div class="detail-section">
...
"""
                ...
                # NEW: Add nearby projects list if available
                nearby_projects = comp_analysis.get('nearby_projects', [])
                if nearby_projects:
                    # Show first 10 projects to avoid overwhelming the display
                    display_projects = nearby_projects[:10]
                    html_content += f"""
...
"""
                    ...
                    if len(nearby_projects) > 10:
                        html_content += f"""
                                <div class="project-item" style="font-style: italic; color: #6c757d;">
                                    <span>... and {len(nearby_projects) - 10} more projects</span>
                                </div>
"""
...
"""
            ...
            # ENHANCED: Demographics Section with Poverty Indicators
            if demo and "error" not in demo:
            ...
            if demo and "error" not in demo:
                ...
                # ENHANCED: Poverty indicator logic
                poverty_rate = demo.get('poverty_rate', 100)
                if poverty_rate < 20.0:
                ...
                # FIXED: Handle invalid income display
                median_income = demo.get('median_household_income', 0)
...
"""
            ...
            # ENHANCED: Location Section with HUD Cross-Reference Link
            if coords:
                encoded_address = urllib.parse.quote_plus(result['address'])
                ...
                # NEW: HUD Poverty Map cross-reference URL
...
if __name__ == "__main__":
    ...
    # Test addresses with some problematic ones that should now work better
    test_addresses = [
    ...
    print(f"\n🧪 Testing FIXED Enhanced Analyzer with {len(test_addresses)} addresses...")
    ...
    print("   • Fixed Census API -666 error handling")
    print("   • Improved poverty rate scoring logic")
    print("   • Enhanced nearby project listings")
    print("   • Added poverty indicators in HTML report")
    print("   • Better category assignment logic")
