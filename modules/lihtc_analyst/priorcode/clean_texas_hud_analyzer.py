...
class TexasHUDAnalyzer:
    ...
    def clean_address(self, address: str) -> List[str]:
        ...
        has_state = ', TX' in address.upper() or ' TX ' in address.upper() or ' TEXAS' in address.upper()
        ...
        # Add "TX" ONLY if not present and no other state mentioned
        if not has_state:
        ...
        if no_apt != address and no_apt:
            ...
            # Only add TX if the original doesn't have state AND the cleaned version doesn't
            if not has_state and ', TX' not in no_apt.upper():
        ...
        for full_word, abbrev in street_replacements:
            ...
            if full_word in address:
                ...
                if new_addr not in variations:
                    ...
                    # Only add state if original doesn't have it
                    if not has_state and ', TX' not in new_addr.upper():
                        variations.append(f"{new_addr}, TX")
        
        # Remove extra formatting like "USA"
        no_usa = re.sub(r',?\s*USA\s*
    ...
    def analyze_address(self, address: str) -> Dict:
        ...
        try:
            ...
            result["poverty_status_text"] = ""
            if result.get("poverty_rate") is not None:
                status_word = "Low Poverty Tract" if result["low_poverty_tract"] else "High Poverty Area"
                result["poverty_status_text"] = f"{status_word} ({result['poverty_rate']:.2f}%)"
            ...
            print(f"  🏆 LIHTC Ranking: {result['lihtc_ranking']['category']} - {result['lihtc_ranking']['description']}")
    ...
    def calculate_lihtc_ranking(self, result: Dict) -> Dict:
        """Calculate LIHTC project ranking: Best, Good, Maybe, Firm No"""
        if 'error' in result:
            return {
                "category": "FIRM NO", 
                "score": 0,
                "description": "Geocoding failed",
                "reasoning": "Cannot determine qualifications - address not found"
            }
        ...
        if qct_qualified and dda_qualified and low_poverty:
            ...
            return {
                ...
                "reasoning": "QCT + DDA qualifications = 30% basis boost available"
            }
            ...
            return {
                ...
                "reasoning": f"{basis_type} qualification (30% basis boost) + Low Poverty area benefits"
            }
        elif qct_qualified or dda_qualified:
            basis_type = "QCT" if qct_qualified else "DDA" 
            poverty_note = f" (High Poverty: {poverty_rate:.1f}%)" if poverty_rate else ""
            return {
                "category": "GOOD",
                "score": 7,
                "description": f"{basis_type} Qualified Only",
                "reasoning": f"{basis_type} qualification = 30% basis boost available{poverty_note}"
            }
        elif low_poverty:
            return {
                "category": "MAYBE",
                ...
                "reasoning": "Low Poverty area may qualify for state incentives, but no federal QCT/DDA benefits"
            }
        else:
            poverty_note = f" (High Poverty: {poverty_rate:.1f}%)" if poverty_rate else ""
            return {
                ...
                "reasoning": f"No QCT, DDA, or Low Poverty qualifications{poverty_note}"
            }
    ...
    def batch_analyze_with_retry(self, addresses: List[str]) -> pd.DataFrame:
        ...
        if failed_addresses:
            ...
            for orig_address in failed_addresses:
                ...
                # General aggressive variations (avoid duplicates)
                addr_upper = orig_address.upper()
                has_state = ', TX' in addr_upper or ' TX ' in addr_upper or ' TEXAS' in addr_upper
                has_zip = re.search(r'\d{5}', orig_address)
                
                # Only add these if they don't already exist
                if not has_state:
                    retry_variations.append(f"{orig_address}, TX")
                    retry_variations.append(f"{orig_address}, Texas")
                
                # Remove specific numbers and try just street (but keep original structure)
                street_only = re.sub(r'^\d+\s+', '', orig_address)
                if street_only != orig_address:
                    retry_variations.append(street_only)
                
                # Remove punctuation
                no_punct = orig_address.replace(',', '').replace('.', '')
                if no_punct != orig_address:
                    retry_variations.append(no_punct)
        ...
        # Sort by LIHTC ranking (Best to Firm No)
        def get_sort_key(row):
            if 'error' in row or pd.isna(row.get('longitude')):
                return (0, row.get('address', ''))  # Failed geocoding goes to bottom
            ranking = row.get('lihtc_ranking', {})
            return (ranking.get('score', 0), row.get('address', ''))
        
        # Convert to list for sorting, then back to DataFrame
        ...
        print(f"📊 Results sorted by LIHTC project viability (Best → Firm No)")
    ...
    def create_summary_report(self, results_df: pd.DataFrame, format_type: str = 'both') -> str:
        ...
        # Ranking summary
        best_count = len(results_df[results_df.apply(lambda row: row.get('lihtc_ranking', {}).get('category') == 'BEST', axis=1)])
        good_count = len(results_df[results_df.apply(lambda row: row.get('lihtc_ranking', {}).get('category') == 'GOOD', axis=1)])
        maybe_count = len(results_df[results_df.apply(lambda row: row.get('lihtc_ranking', {}).get('category') == 'MAYBE', axis=1)])
        firm_no_count = len(results_df[results_df.apply(lambda row: row.get('lihtc_ranking', {}).get('category') == 'FIRM NO', axis=1)])
...
🥇 BEST Projects: {best_count} ({best_count/total*100:.1f}%)
🥈 GOOD Projects: {good_count} ({good_count/total*100:.1f}%)
🥉 MAYBE Projects: {maybe_count} ({maybe_count/total*100:.1f}%)
❌ FIRM NO Projects: {firm_no_count} ({firm_no_count/total*100:.1f}%)
...
📋 DETAILED RESULTS BY ADDRESS (Sorted by LIHTC Project Viability):
"""
        
        # Add detailed results for each address (now sorted by ranking)
        for _, row in results_df.iterrows():
            if pd.isna(row.get('longitude')):
                text_report += f"\n❌ {row['address']}: GEOCODING FAILED"
                text_report += f"\n   Google Maps: {row.get('google_maps_link', 'N/A')}"
                text_report += f"\n   Ranking: {row.get('lihtc_ranking', {}).get('category', 'FIRM NO')} - {row.get('lihtc_ranking', {}).get('description', 'Failed geocoding')}"
            else:
                ranking = row.get('lihtc_ranking', {})
                ranking_emoji = {"BEST": "🥇", "GOOD": "🥈", "MAYBE": "🥉", "FIRM NO": "❌"}.get(ranking.get('category'), "❓")
                
                text_report += f"\n{ranking_emoji} {row['address']}"
                text_report += f"\n   Ranking: {ranking.get('category')} - {ranking.get('description')}"
                ...
                # Always show poverty information if available
                if row.get('poverty_status_text'):
                    text_report += f"\n   Poverty Status: {row['poverty_status_text']}"
                elif not pd.isna(row.get('poverty_rate')):
...
\\line
Successfully Geocoded: \\cf3\\b {successful}\\b0\\cf1 ({successful/total*100:.1f} percent)
\\line
Failed Geocoding: \\cf2\\b {total - successful}\\b0\\cf1
\\line
\\line

\\b\\fs22 LIHTC PROJECT RANKINGS:\\b0\\fs20
\\line
\\cf3 BEST Projects: \\b {best_count}\\b0\\cf1 ({best_count/total*100:.1f} percent)
\\line
\\cf6 GOOD Projects: \\b {good_count}\\b0\\cf1 ({good_count/total*100:.1f} percent)
\\line
\\cf4 MAYBE Projects: \\b {maybe_count}\\b0\\cf1 ({maybe_count/total*100:.1f} percent)
\\line
\\cf2 FIRM NO Projects: \\b {firm_no_count}\\b0\\cf1 ({firm_no_count/total*100:.1f} percent)
\\line
\\line

\\b\\fs22 QCT (QUALIFIED CENSUS TRACT) STATUS:\\b0\\fs20
\\line
\\cf3 QCT Qualified: \\b {qct_qualified}\\b0\\cf1 ({qct_qualified/total*100:.1f} percent)
\\line
\\cf2 Non-QCT: \\b {total - qct_qualified}\\b0\\cf1 ({(total-qct_qualified)/total*100:.1f} percent)
\\line
\\line

\\b\\fs22 DDA (DIFFICULT DEVELOPMENT AREA) STATUS:\\b0\\fs20
\\line
\\cf3 DDA Qualified: \\b {dda_qualified}\\b0\\cf1 ({dda_qualified/total*100:.1f} percent)
\\line
\\cf2 Non-DDA: \\b {total - dda_qualified}\\b0\\cf1 ({(total-dda_qualified)/total*100:.1f} percent)
\\line
\\line

\\b\\fs22 LOW POVERTY TRACT STATUS (≤20 percent):\\b0\\fs20
\\line
\\cf3 Low Poverty: \\b {low_poverty}\\b0\\cf1 ({low_poverty/total*100:.1f} percent)
\\line
\\cf2 Higher Poverty: \\b {total - low_poverty}\\b0\\cf1 ({(total-low_poverty)/total*100:.1f} percent)
\\line
\\line
...
\\line
• QCT Qualified properties can claim 30 percent basis boost
\\line
• DDA Qualified properties can claim 30 percent basis boost
\\line
• Low Poverty areas may qualify for additional state incentives
\\line
• Triple qualified sites have maximum flexibility for tax credit structures
...
\\b\\fs22 DETAILED RESULTS BY ADDRESS (Sorted by LIHTC Project Viability):\\b0\\fs20
\\line
"""
        ...
        for _, row in results_df.iterrows():
            if pd.isna(row.get('longitude')):
                ...
                rtf_content += f"\\line    Ranking: {row.get('lihtc_ranking', {}).get('category', 'FIRM NO')} - {row.get('lihtc_ranking', {}).get('description', 'Failed geocoding')}"
            else:
                ranking = row.get('lihtc_ranking', {})
                # Color coding by ranking
                ranking_color = {"BEST": "\\cf3", "GOOD": "\\cf6", "MAYBE": "\\cf4", "FIRM NO": "\\cf2"}.get(ranking.get('category'), "\\cf1")
                
                rtf_content += f"\\line{ranking_color}\\b {row['address']}\\b0\\cf1"
                rtf_content += f"\\line    Ranking: \\b {ranking.get('category')} - {ranking.get('description')}\\b0"
                # Fix % symbol encoding for Mac RTF compatibility
                compact_status = row.get('compact_status', 'Status unavailable')
                compact_status_rtf = compact_status.replace('%', ' percent')
                rtf_content += f"\\line    \\cf4\\b {compact_status_rtf}\\b0\\cf1"
                ...
                # Always show poverty information if available
                if row.get('poverty_status_text'):
                    poverty_text_rtf = row['poverty_status_text'].replace('%', ' percent')
                    rtf_content += f"\\line    Poverty Status: \\b {poverty_text_rtf}\\b0"
                elif not pd.isna(row.get('poverty_rate')):
                    poverty_status = "Low Poverty Tract" if row.get('low_poverty_tract') else "High Poverty Area"
                    rtf_content += f"\\line    Poverty Status: \\b {poverty_status} ({row['poverty_rate']:.2f} percent)\\b0"
                if row.get('matched_address') and row['matched_address'] != row['address']:
                    rtf_content += f"\\line    (Matched as: {row['matched_address']})"
                rtf_content += f"\\line    Google Maps: {row.get('google_maps_link', 'N/A')}"
                rtf_content += f"\\line    Reasoning: {ranking.get('reasoning', 'N/A')}"
                rtf_content += "\\line"
...
if __name__ == "__main__":
    ...
    if analyzer.load_hud_data():
        ...
        print(f"🥉 MAYBE Projects: {maybe_count}")
        print(f"❌ FIRM NO Projects: {firm_no_count}")
        
        # Show top 3 projects
        if best_count > 0 or good_count > 0:
            print(f"\n🌟 TOP RECOMMENDED PROJECTS:")
            top_projects = results_df[results_df.apply(lambda row: row.get('lihtc_ranking', {}).get('category') in ['BEST', 'GOOD'], axis=1)]
            for i, (_, row) in enumerate(top_projects.head(3).iterrows(), 1):
                ranking = row.get('lihtc_ranking', {})
                ranking_emoji = {"BEST": "🥇", "GOOD": "🥈"}.get(ranking.get('category'), "")
                print(f"   {i}. {ranking_emoji} {row['address']}")
                print(f"      → {ranking.get('description', 'N/A')}")
        
        print("\n✅ Analysis complete! Check the generated files:")
        print("   - CSV file: Contains all detailed results (sorted by ranking)")
        print("   - TXT file: Text summary report with Google Maps links")
        print("   - RTF file: Rich formatted report (open in Word/Pages)")
        print("\n📱 Pro Tip: Click the Google Maps links to verify addresses!")
