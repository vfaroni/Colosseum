#!/usr/bin/env python3
"""
Direct test of extraction methods on Sunset Gardens without full module import
"""
...
def test_live_sunset_gardens():
    """Test the fixed extraction on actual Sunset Gardens rent roll"""
    
    print("=== LIVE TEST: SUNSET GARDENS EXTRACTION ===")
    print("Testing fixed extraction logic on real rent roll...")
    ...
    try:
        ...
        if header_info['success']:
            ...
            if len(clean_data) > 0:
                ...
                result = {
                    "Number of Units": str(total_units),
                    "Avg In Place Rents": f"${avg_rent:,.0f}" if avg_rent > 0 else "",
                }
                ...
                expected = {
                    "Number of Units": "102",
                    "Avg In Place Rents": "$2,243",
                    "# 1 Bed Units": "53",
                    "# 2 Bed Units": "49", 
                    "1 Bed Current Rents": "$1,968",
                    "2 Bed Current Rents": "$2,507"
                }
                ...
                for field, expected_value in expected.items():
                    ...
                    if field == "Number of Units":
                        if actual_value == expected_value:
                            print(f"✅ {field}: {actual_value} (PERFECT MATCH)")
                        else:
                        ...
                        if actual_value:
                            ...
                            if abs(actual_num - expected_num) <= tolerance:
                                print(f"✅ {field}: {actual_value} (within ${tolerance} tolerance)")
                            else:
                                print(f"❌ {field}: Expected ~{expected_value}, got {actual_value}")
                                success = False
                        else:
                        ...
                        if actual_value:
                            ...
                            if abs(actual_num - expected_num) <= tolerance:
                                print(f"✅ {field}: {actual_value} (within {tolerance} unit tolerance)")
                            else:
                                print(f"❌ {field}: Expected ~{expected_value}, got {actual_value}")
                                success = False
                        else:
                ...
                if success:
                    ...
                    print("Fixed extraction logic correctly processes Sunset Gardens!")
                    print("All metrics are within acceptable tolerances.")
                else:
                    print("❌ LIVE TEST FAILED!")
                    print("Some metrics are outside acceptable ranges.")
