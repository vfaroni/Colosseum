...
"""
Houston Harris County Parcel Data Downloader
Downloads parcel boundary data for Harris County (Houston metro)
Output: /data_sets/Texas/Parcels/houston_parcels.geojson
"""
...
def download_harris_county_parcels():
    """Download Harris County parcel data"""
    print("🏢 DOWNLOADING HOUSTON HARRIS COUNTY PARCEL DATA")
    print("=" * 60)
    
    # Harris County parcel data URLs (try multiple sources)
    sources = [
        {
            "name": "Harris County HCAD Parcels",
            "url": "https://services.arcgis.com/su8ic9KbA6N1Gf9u/arcgis/rest/services/HCAD_Parcels/FeatureServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=geojson"
        },
        {
            "name": "Harris County GIS Parcels",
            "url": "https://gis-hcad.opendata.arcgis.com/api/download/v1/items/4c3b3e5a6d374c8ab09e4b4b2bf8b8b1/geojson?layers=0"
        },
        {
            "name": "Houston Area GIS Parcels",
            "url": "https://services2.arcgis.com/su8ic9KbA6N1Gf9u/arcgis/rest/services/Property_Parcels/FeatureServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=geojson"
        }
    ]
            ...
            try:
                ...
                if 'features' in data:
                    ...
                    for feature in data['features']:
                        ...
                        parcel_id = (props.get('HCAD_NUM') or
                                   ...
                                   props.get('FID') or 
                                   f"HARRIS_{len(standardized_features)}")
                    ...
                    output_file = os.path.join(output_dir, "houston_parcels.geojson")
                    ...
                    metadata = {
                        ...
                        "coverage": "Harris County (Houston metro)",
                        "coordinate_system": "WGS84 (EPSG:4326)"
                    }
                    
                    metadata_file = os.path.join(output_dir, "houston_parcels_metadata.json")
                    with open(metadata_file, 'w') as f:
                        json.dump(metadata, f, indent=2)
                    
                    print(f"📋 Saved metadata to: {metadata_file}")
                    print(f"🎯 SUCCESS: Houston parcel data ready for bulk processing")
                    return True
...
def test_data_quality():
    ...
    output_dir = "/Users/williamrice/Library/CloudStorage/Dropbox-HERR/Bill Rice/Colosseum/data_sets/Texas/Parcels"
    data_file = os.path.join(output_dir, "houston_parcels.geojson")
    ...
    try:
        ...
        # Test coordinate ranges (should be in Houston area)
        bounds = gdf.total_bounds  # [minx, miny, maxx, maxy]
        print(f"🌐 Bounding box: {bounds}")
        
        # Houston should be roughly: lng -96 to -94.5, lat 29 to 30.5
        if (-96.5 < bounds[0] < -94) and (-96.5 < bounds[2] < -94) and (28.5 < bounds[1] < 31) and (28.5 < bounds[3] < 31):
            print("✅ Coordinates appear to be in Houston region")
        else:
            print("⚠️ Coordinates may be outside expected Houston region")
...
if __name__ == "__main__":
    print("🚀 Starting Houston Parcel Data Download")
    
    success = download_harris_county_parcels()
    
    if success:
        test_data_quality()
        print("\n🏆 HOUSTON PARCEL DATA DOWNLOAD COMPLETE")
        print("Ready for bulk parcel analysis!")
    else:
        print("\n💥 HOUSTON PARCEL DATA DOWNLOAD FAILED")
