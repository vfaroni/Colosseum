...
class NominatimLPSTGeocoder:
    """
    Precise geocoding using free OpenStreetMap Nominatim service
    ...
    def __init__(self):
        ...
        # Nominatim base URL and headers
        self.nominatim_url = "https://nominatim.openstreetmap.org/search"
        self.headers = {
            'User-Agent': 'DMarcoEnvironmentalScreening/1.0 (environmental.screening@structured-consultants.com)'
        }
        ...
        for i, site in enumerate(sites_data):
            ...
            if len(corners) == 4:
                processed_site = {
                    ...
                    'city': self.extract_city_from_address(site.get('Address', '')),
    ...
    def geocode_address_nominatim(self, address, city, state='TX'):
        ...
        try:
            ...
            response = requests.get(self.nominatim_url, params=params, headers=self.headers, timeout=15)
            ...
            if response.status_code == 200:
                ...
                if len(data) > 0:
                    ...
                    confidence = self.calculate_nominatim_confidence(result, clean_address)
                    ...
                    time.sleep(1.1)  # Be respectful to free service - 1 request per second
    ...
    def calculate_nominatim_confidence(self, result, query_address):
        """Calculate confidence score based on Nominatim result quality"""
        confidence = 0.5  # Base confidence
        
        # Check if result contains key address components
        address = result.get('address', {})
        display_name = result.get('display_name', '').upper()
        query_upper = query_address.upper()
        
        # Boost confidence if street name matches
        if 'road' in address or 'street' in display_name:
            confidence += 0.2
        
        # Boost confidence if city matches
        if any(city in display_name for city in self.target_cities):
            confidence += 0.2
        
        # Boost confidence if state is Texas
        if 'TEXAS' in display_name or 'TX' in display_name:
            confidence += 0.1
    ...
    def clean_address_for_geocoding(self, address):
        ...
        # Handle "NAN" addresses - use just city for general area
    ...
    def create_lpst_database_file(self, distance_results):
        ...
        headers = [
            ...
            'Latitude', 'Longitude', 'Geocoding_Confidence', 'Geocoding_Source'
        ]
        ...
        for result in distance_results:
            row = [
                ...
                result['coordinates'].get('source', 'OpenStreetMap')
            ]
    ...
    def run_precise_lpst_analysis(self):
        ...
        print("Using Free OpenStreetMap Nominatim Service")
        print("=" * 55)
        ...
        for idx, row in active_lpst_df.iterrows():
            try:
                ...
                # Geocode the address
                geocode_result = self.geocode_address_nominatim(address, city)
                ...
                if (len(geocoded_lpst_sites) + len(self.geocode_failures)) % 25 == 0:
                    total_processed = len(geocoded_lpst_sites) + len(self.geocode_failures)
                    success_rate = len(geocoded_lpst_sites) / total_processed * 100 if total_processed > 0 else 0
        ...
        final_output = {
            ...
            'analysis_type': 'Precise LPST Geocoding & Distance Analysis (OpenStreetMap)',
...
def main():
    ...
    geocoder = NominatimLPSTGeocoder()
    results = geocoder.run_precise_lpst_analysis()
    
    if results:
        print("\n🎉 PRECISE LPST ANALYSIS COMPLETE!")
        print("✅ Active LPST sites geocoded with OpenStreetMap Nominatim")
        ...
        print("\n❌ Analysis failed - check network connection and try again")
